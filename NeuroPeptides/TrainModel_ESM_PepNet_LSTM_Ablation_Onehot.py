...
from ESM_PepNet_LSTM_Ablation_Onehot import AIMP
import warnings
...
for epoch in range(epochs):
    ...
    for index, batch in enumerate(train_dataloader):
        ...
        # one_hot_embedding = get_onehot_features(batch_protein_sequences) 消融onehot
        # one_hot_tensor = torch.tensor(one_hot_embedding, device=device, dtype=torch.float)   消融onehot
    ...
    with torch.no_grad():
        ...
        for index, batch in enumerate(val_dataloader):
            ...
            outputs = model(batch_protein_sequences)   #保留tcn_features特征，用于做特征可视化
    ...
    if valid_acc >= best_acc:
        best_acc = valid_acc
        torch.save(model.state_dict(), f"my_best_model_2_lstm_data_split_train_ablation_onehot.pth")
