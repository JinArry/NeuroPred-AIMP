...
from ESM_PepNet_Transfomer import AIMP
import warnings
...
model = AIMP(pre_feas_dim=1280, feas_dim=21, hidden=1024, n_transformer=2, dropout=0.5)
model = model.cuda()
...
for epoch in range(epochs):
    ...
    if valid_acc >= best_acc:
        best_acc = valid_acc
        torch.save(model.state_dict(), f"esm_pepnet.pth")

    print(
        f'Epoch {epoch}, Train Loss: {np.mean(train_epoch_loss):.4f}, Train Acc: {train_acc:.4f}, Train Sensitivity: {train_sensitivity:.4f}, Train Specificity: {train_specificity:.4f}, Train AUC: {train_auc:.4f}')
    print(
        f'Epoch {epoch}, Valid Loss: {np.mean(valid_epoch_loss):.4f}, Valid Acc: {valid_acc:.4f}, Valid Sensitivity: {val_sensitivity:.4f}, Valid Specificity: {val_specificity:.4f}, Valid AUC: {val_auc:.4f}')
