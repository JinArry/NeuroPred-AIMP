...
class AIMP(torch.nn.Module):
    ...
    def forward(self, peptide_sequence):
        ...
        pre_feas = self.pre_embedding(pre_feas.permute(0, 2, 1))

        # tcn_out = self.tcn(feas)
        # tcn_out = self.tcn_res(torch.cat([tcn_out, feas], dim=-1).permute(0, 2, 1)).permute(0, 2, 1)

        # feas_em = self.embedding(torch.cat([pre_feas, tcn_out], dim=-1).permute(0, 2, 1)).permute(0, 2, 1) 消融onehot

        feas_em = self.embedding(pre_feas).permute(0, 2, 1)
        ...
        # return out,tcn_out, bert_output,pre_feas,feas_em,transformer_out 消融onehot
        return out