...
class AIMP(torch.nn.Module):
    ...
    def forward(self, peptide_sequence, one_hot_embedding):
        ...
        # bert_output = self.esm(peptide_sequence)  消融实验esm
        # pre_feas = self.bn[0](bert_output.permute(0, 2, 1)).permute(0, 2, 1) 消融实验esm
        feas = self.bn[1](one_hot_embedding.permute(0, 2, 1)).permute(0, 2, 1)

        # pre_feas = self.pre_embedding(pre_feas.permute(0, 2, 1)).permute(0, 2, 1) 消融实验esm

        tcn_out = self.tcn(feas)
        tcn_out = self.tcn_res(torch.cat([tcn_out, feas], dim=-1).permute(0, 2, 1))

        feas_em = self.embedding(tcn_out).permute(0, 2, 1)
