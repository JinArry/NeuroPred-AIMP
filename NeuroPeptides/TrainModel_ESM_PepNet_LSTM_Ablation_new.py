...
model = model.cuda(device)  # 修改为cuda:1
...
for epoch in range(epochs):
    ...
    for index, batch in enumerate(train_dataloader):
        ...
        label = torch.nn.functional.one_hot(batch_labels, num_classes=2).float().to(device)
        ...
        correct += (train_argmax == batch_labels.to(device)).sum().item()
    ...
    with torch.no_grad():
        ...
        for index, batch in enumerate(val_dataloader):
            ...
            correct += (val_argmax == batch_labels.to(device)).sum().item()
    ...
    if valid_acc >= best_acc:
        best_acc = valid_acc
        torch.save(model.state_dict(), f"my_best_model_2_lstm_data_split_train_Ablation_lstm.pth")
